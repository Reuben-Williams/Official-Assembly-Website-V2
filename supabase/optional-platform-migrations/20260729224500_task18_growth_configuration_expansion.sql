create or replace function public.builder_apply_growth_configuration(
  p_site_id uuid,
...
begin
  if p_module_id !~ '^[a-z][a-z0-9-]*(\.[a-z][a-z0-9-]*)+$'
    ...
    or (p_module_id, p_module_version) not in (
      ('growth.customers', '1.0.0'),
      ('growth.leads', '1.0.0'),
      ('growth.dashboard', '2.0.0'),
      ('growth.customers', '1.1.0'),
      ('growth.leads', '1.1.0'),
      ('growth.dashboard', '2.1.0'),
      ('growth.bookings', '1.0.0'),
      ('growth.messaging', '1.0.0'),
      ('growth.automations', '1.0.0'),
      ('growth.ai', '1.0.0'),
      ('growth.chat', '1.0.0'),
      ('growth.campaigns', '1.0.0')
    )
  ...
  v_command_type := case
    when (p_module_id, p_module_version) in (
      ('growth.customers', '1.1.0'),
      ('growth.leads', '1.1.0'),
      ('growth.dashboard', '2.1.0')
    ) then p_module_id || '.configure-v2'
    else p_module_id || '.configure'
  end;
