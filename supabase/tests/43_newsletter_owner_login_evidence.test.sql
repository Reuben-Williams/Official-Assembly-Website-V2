...
select has_table('public', 'builder_newsletter_auth_login_occurrences', 'owner login occurrences exist');
select has_table('public', 'builder_newsletter_auth_login_evidence', 'owner login evidence exists');
select has_table('public', 'builder_newsletter_auth_login_recovery_commands', 'owner login recovery commands exist');
select has_column('public', 'builder_newsletter_site_jobs', 'auth_login_occurrence_id', 'site jobs bind login occurrences');
select is((select relrowsecurity from pg_class where oid = 'public.builder_newsletter_auth_login_occurrences'::regclass), true, 'occurrences enforce RLS');
select is((select relrowsecurity from pg_class where oid = 'public.builder_newsletter_auth_login_evidence'::regclass), true, 'evidence enforces RLS');
select ok(
...
select is((select response ->> 'status' from pg_temp.owner_login_occurrence), 'queued', 'an owner login occurrence queues durable work');
select is(
...
select is((select response #>> '{jobs,0,kind}' from pg_temp.owner_login_claim), 'newsletter.auth_login.reconcile', 'owner evidence claims while independent of outbound sending');
select is((select (response #>> '{jobs,0,attemptCount}')::integer from pg_temp.owner_login_claim), 1, 'the bounded claim records one attempt');
reset role;
