...
That presentation mode suppresses `PublicFormCard`’s duplicate visual eyebrow and `<h3>` header for both the live form and the unavailable fallback. The first-section `h1` receives a stable DOM ID. The live state exposes exactly one named form landmark through a wrapper with `role="form"` and `aria-labelledby` referencing that `h1`; the nested native form remains unnamed as a landmark while its fieldset legend continues to name the control group. The unavailable state uses a non-form wrapper with `role="group"` and the same `aria-labelledby` reference, while its existing status/fallback message remains announced normally. The required-fields guidance remains adjacent to the controls. No `h1 → h3` skip or second signup introduction is permitted.
...
- Add live and unavailable-state accessible-name tests for `newsletter-page-first` presentation: no public-form-card eyebrow or `h3`; one `role="form"` landmark whose accessible name resolves from the first-section `h1`; the existing legend retained for the live control group; and one unavailable `role="group"` whose accessible name resolves from the same heading.
